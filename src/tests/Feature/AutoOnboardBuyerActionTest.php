...
beforeEach(function() {
    ...
    $this->checkin_code = '9b182ba6-842f-4531-9d38-920e5a904358';
});
...
test('auto onboard buyer action with no contacts and zero inventory', function (array $kyc_inputs, string $project_code) {
    ...
    expect($voucher->checkin_code)->toBe($this->checkin_code);
...
//    $voucher_code = ((object) $response->getSession()->get('voucher'))->reference_code;
    $voucher = Voucher::where('checkin_code', $this->checkin_code)->first();;
    expect($order = $voucher->getOrder())->toBeInstanceOf(Order::class);
    expect($order->buyer)->toBeInstanceOf(Buyer::class);
    expect($order->buyer->contact)->toBeNull();
    expect($order->state)->toBeInstanceOf(ProcessedPendingConfirmation::class);
